...
export default class Model {
    ...
    drawOnCanvas(ctx, ctxWidth, ctxHeight) {
        ...
        if (this.isWon()) {
            ctx.font = "35px Arial";
            ctx.fillStyle = "black";
            ctx.fillText("You've reached the goal state", ctxWidth*0.2, ctxHeight*0.40);
        }
    ...
    drawQueue(ctx, queueNum, marbX, marbY, marbleRadius) {
        ...
        ctx.fillStyle = "black";
        ctx.font = "15px Arial";
        ctx.fillText("queue_"+queueNum, marbX - 20, marbY-marbleRadius-10, 70);
        ...
        ctx.stroke();
        ctx.moveTo(marbX+(numMarbs+1)*2*marbleRadius-10+1.5*marbleRadius-10, marbY-10);
        ctx.lineTo(marbX+(numMarbs+1)*2*marbleRadius+marbleRadius, marbY);
        ctx.stroke();
        ctx.moveTo(marbX+(numMarbs+1)*2*marbleRadius-10+1.5*marbleRadius-10, marbY+10);
        ctx.lineTo(marbX+(numMarbs+1)*2*marbleRadius+marbleRadius, marbY);
        ...
        if (queue.length == 0) {
            ...
            ctx.fillStyle = "white";
            ctx.fillRect(marbX-marbleRadius-1, marbY-marbleRadius-1, 3, marbleRadius*2);
            ctx.fillRect(marbX+marbleRadius-1, marbY-marbleRadius-1, 3, marbleRadius*2);
            return;
        }
        ...
        for (i=0; i<queue.length; i++) {
            this.drawMarble(ctx, marbX+i*2*marbleRadius, marbY, marbleRadius, queue.items[i].colour);
        }
        ctx.fillStyle = "black";
