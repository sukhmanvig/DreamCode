...
const SortGameJS = () => {
  ...
  document.querySelector("#init-array").addEventListener("click", () => {
    ...
    if (isNaN(entries)) entries = 12;
    var rangemax = parseInt(
      document.querySelector("#init-array-rangemax").value
    );
    if (isNaN(entries)) entries = 12;
    console.log(
      "Created array of size " + entries + " with values from 0 to " + rangemax
    );
    ...
    for (var k = 0; k < array.length; k++) {
      tablehtml +=
        "<td>" +
        `<button id=swap_${k} class="game_button">Swap #${k}</button>` +
        "</td>";
    ...
    for (var k = 0; k < array.length; k++) {
      document
        .querySelector(`#swap_${k}`)
        .addEventListener("click", swaplisteners[k]);
    }
    // Enable hints
    ...
    document.querySelector("#swapmode").className =
      "game_button sorthint active";
  ...
  document.querySelector("#sortstep").addEventListener("click", () => {
    ...
    if (gamestate.autosolve === "Selection") {
      ...
      document.querySelector(
        "#gamehint"
      ).innerHTML = `Swap element #${gamestate.autosolvehint[0]} with #${kmin}. ${gamestate.autosolvehint[2]} steps total. ${innersteps} steps for this outer loop.`;
      disableSwapInput();
      for (var k = 0; k < gamestate.autosolvehint[0]; k++)
        document.querySelector(`#swap_${k}`).className = "game_button partial";
      document.querySelector(`#swap_${gamestate.autosolvehint[0]}`).className =
        "game_button hinted";
      ...
      for (var k = 0; k < array.length; k++) {
        if (k <= gamestate.autosolvehint[0])
          document.querySelector(`#swap_${k}`).className =
            "game_button partial";
        else document.querySelector(`#swap_${k}`).className = "game_button";
      }

      if (gamestate.won) {
        document.querySelector(
          "#gamehint"
        ).innerHTML += `Swap element #${gamestate.autosolvehint[0]} with #${gamestate.autosolvehint[1]}. Finished with ${gamestate.autosolvehint[2]} steps total.`;
      }
      ...
      var bubbles = 0;
      for (
        var k = gamestate.autosolvehint[0];
        k > 0 && elem > array[k - 1];
        k--
      ) {
        bubbles++;
      ...
      for (var k = 0; k < array.length; k++) {
        ...
        if (
          k <= gamestate.autosolvehint[0] &&
          k >= gamestate.autosolvehint[0] - bubbles
        )
      ...
      document.querySelector(
        "#gamehint"
      ).innerHTML = `${bubbles} bubbles required to move element to position ${
        gamestate.autosolvehint[0] - bubbles
      }.`;
      ...
      document.querySelector(
        "#gamehint"
      ).innerHTML = `"Inserted" element #${gamestate.autosolvehint[0]} at position #${k}. ${gamestate.autosolvehint[1]} bubbles total.`;
      gamestate.autosolvehint[0]++;
      for (var k = 0; k < array.length; k++) {
        if (k <= gamestate.autosolvehint[0])
          document.querySelector(`#swap_${k}`).className =
      ...
      /* Perform a Quicksort */
      var ini = gamestate.autosolvehint[0][0];
      var fin = gamestate.autosolvehint[0][1];
      if (fin - ini <= 2) {
        // Base case
        if (fin - ini > 1) {
          if (array[fin - 1] < array[ini]) {
            clickSwapInput(ini)();
            clickSwapInput(fin - 1)();
          } else {
        ...
        for (var k = 0; k < array.length; k++) {
          if (k >= ini && k < fin)
            ...
            document.querySelector(`#swap_${k}`).className =
              "game_button offscope";
        }
        gamestate.autosolvehint[0] = gamestate.autosolvehint[4].pop();
        document.querySelector(
          "#gamehint"
        ).innerHTML = `Base case with ${gamestate.autosolvehint[4].length} calls remaining. Starting Quick Sort from [${gamestate.autosolvehint[0][0]},${gamestate.autosolvehint[0][1]}].`;
      } else {
        var pivotindex = Math.trunc((ini + fin) / 2);
        var pivotvalue = array[pivotindex];
        clickSwapInput(ini)();
        clickSwapInput(pivotindex)();
        var lowercount = 1;
        ...
        document.querySelector(
          "#gamehint"
        ).innerHTML = `Perform Pivot on ${pivotvalue} from #${pivotindex}`;
        for (var k = 0; k < array.length; k++) {
          if (k >= ini && k < fin)
        ...
        document.querySelector(`#swap_${pivotindex}`).className =
          "game_button hinted";
        gamestate.autosolve = "QuickPivot";
      }
      for (var k in gamestate.autosolvehint[5]) {
        document.querySelector(
          `#swap_${gamestate.autosolvehint[5][k]}`
        ).className = "game_button partial";
      }
    } else if (gamestate.autosolve === "QuickPivot") {
      /* Perform a Quicksort */
      var ini = gamestate.autosolvehint[0][0];
      var fin = gamestate.autosolvehint[0][1];
      ...
      document.querySelector(
        "#gamehint"
      ).innerHTML = `Performed partitions. There are ${lowercount} within ${pivotvalue}, the pivot value.`;
    ...
    } else if (gamestate.autosolve === "QuickPartition") {
      /* Perform a Quicksort */
      var ini = gamestate.autosolvehint[0][0];
      var fin = gamestate.autosolvehint[0][1];
      var pivotvalue = gamestate.autosolvehint[2];
      ...
      clickSwapInput(ini + lowercount - 1)();
      clickSwapInput(ini)();
      ...
      document.querySelector(
        "#gamehint"
      ).innerHTML = `Declared partitions. There are ${lowercount} within ${pivotvalue}, the pivot value.`;
    ...
    } else if (gamestate.autosolve === "QuickRecursive") {
      /* Perform a Quicksort */
      var ini = gamestate.autosolvehint[0][0];
      var fin = gamestate.autosolvehint[0][1];
      var lowercount = gamestate.autosolvehint[3];
      ...
      document.querySelector(
        "#gamehint"
      ).innerHTML = `${gamestate.autosolvehint[4].length} calls remaining. Starting Quick Sort from [${gamestate.autosolvehint[0][0]},${gamestate.autosolvehint[0][1]}].`;
      for (var k = 0; k < array.length; k++) {
        if (
          k >= gamestate.autosolvehint[0][0] &&
          k < gamestate.autosolvehint[0][1]
        )
          document.querySelector(`#swap_${k}`).className = "game_button hinted";
        else
          document.querySelector(`#swap_${k}`).className =
      ...
      for (var x in array) {
        bucketcount[
          Math.trunc(array[x] / gamestate.autosolvehint[1]) % 10
        ].push(array[x]);
        count++;
        moves++;
      }
      ...
      document.querySelector("#gamehint").innerHTML =
        "The distribution of digits in the place value " +
        gamestate.autosolvehint[1] +
        " is: [";
      for (var k = 0; k < bucketcount.length; k++) {
        document.querySelector("#gamehint").innerHTML +=
          bucketcount[k].length + " ";
      }
      document.querySelector("#gamehint").innerHTML += "]";
      gamestate.autosolve = "Radix2";
    } else if (gamestate.autosolve === "Radix2") {
      /* Perform a Radixsort */
      // Update array in place and view. The loop is required only as an artifact.
      for (x in array) array[x] = gamestate.autosolvehint[2][x];
      redisplay_array();
      document.querySelector("#gamehint").innerHTML =
        "Performed " +
        gamestate.autosolvehint[3] +
        " transfers from bucket to array with " +
        moves +
        " transfers total.";
  ...
  document.querySelector("#selectionsort").addEventListener("click", () => {
    ...
    document.querySelector("#selectionsort").className =
      "game_button sorthint active";
  });
  ...
  document.querySelector("#insertionsort").addEventListener("click", () => {
    ...
    document.querySelector("#insertionsort").className =
      "game_button sorthint active";
  });
  ...
  document.querySelector("#quicksort").addEventListener("click", () => {
    ...
    document.querySelector(
      "#gamehint"
    ).innerHTML = `Starting Quick Sort from [${gamestate.autosolvehint[0][0]},${gamestate.autosolvehint[0][1]}] `;
    var elems = document.getElementsByClassName("sorthint");
    for (var k = 0; k < elems.length; k++) {
      elems[k].className = "game_button sorthint";
    }
    document.querySelector("#quicksort").className =
      "game_button sorthint active";
  });
  ...
  document.querySelector("#radixsort").addEventListener("click", () => {
    ...
    var longestrep = 0;
    for (var x in array) {
      if (array[x] > longestrep) longestrep = array[x];
    }
    ...
    document.querySelector("#radixsort").className =
      "game_button sorthint active";
  });
  ...
  function clickSwapInput(k1) {
    return function () {
      var k = k1;
      console.log(
        "State:" + gamestate.swapinputs + " ; " + "Key clicked: " + k
      );
      ...
      if (gamestate.swapinputs[0] === null) {
        ...
        document.querySelector("#gamehint").innerHTML =
          "Selecting " + k + " for swapping. Click on same button to undo.";
        ...
        console.log(
          "Swapped elements of position " +
            gamestate.swapinputs[0] +
            " and " +
            k
        );
        // Update view
        ...
        document.querySelector(`#swap_${gamestate.swapinputs[0]}`).className =
          "game_button";
        document.querySelector("#gamehint").innerHTML =
          "Swapped element #" +
          gamestate.swapinputs[0] +
  ...
  function quicksort(array, ini, fin) {
    ...
    return (
      count +
      quicksort(array, ini, ini + lowercount - 1) +
      quicksort(array, ini + lowercount, fin)
    );
  }
  ...
  function radixsort_N(array) {
    ...
    while (longestrep !== 0) {
      ...
      // Update array in place and view. The loop is required only as an artifact.
      for (x in array) array[x] = nextarray[x];
      redisplay_array();
      document.querySelector("#gamehint").innerHTML =
        "Performed " + count + " transfers from bucket to array";
